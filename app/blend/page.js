...
export default function BlendPage() {
  ...
  const detectCondition = (input) => {
    ...
    const conditionMap = {
      'menopause': { weight: 0, keywords: ['menopause', 'menopausal', 'peri-menopause', 'perimenopause', 'post-menopause', 'hot flash', 'hot flashes', 'hormone', 'hormonal', 'estrogen', 'progesterone', 'mood swing', 'brain fog', 'hair loss', 'weight gain'] },
      'stress': { weight: 0, keywords: ['stress', 'stressed', 'overwhelm', 'anxiety', 'anxious', 'panic', 'worry', 'fear', 'tension', 'burnout', 'exhausted', 'insomnia', 'sleepless', 'can\'t sleep'] },
      'headache': { weight: 0, keywords: ['headache', 'migraine', 'head pain', 'throbbing', 'pounding', 'cluster headache', 'sinus headache', 'aura', 'nausea'] },
      'sciatic': { weight: 0, keywords: ['sciatica', 'sciatic nerve', 'shooting pain', 'radiating leg', 'lower back', 'leg pain', 'nerve pain'] },
      'joint': { weight: 0, keywords: ['joint', 'joints', 'joint pain', 'arthritis', 'stiff joints', 'knee pain', 'hip pain', 'swelling'] },
      'insomnia': { weight: 0, keywords: ['insomnia', 'sleepless', 'can\'t sleep', 'trouble sleeping', 'wake up', 'early morning wake', 'toss and turn'] },
      'musclepain': { weight: 0, keywords: ['muscle', 'muscles', 'muscle pain', 'sore muscles', 'cramps', 'straining', 'DOMS', 'workout soreness'] },
      'digestion': { weight: 0, keywords: ['digestion', 'bloating', 'gas', 'nausea', 'upset stomach', 'indigestion', 'IBS', 'irregular bowel'] },
      'shoulder': { weight: 0, keywords: ['shoulder', 'shoulder pain', 'frozen shoulder', 'rotator cuff', 'can\'t reach', 'above head pain'] },
      'glucose': { weight: 0, keywords: ['glucose', 'blood sugar', 'diabetes', 'sugar level', 'insulin resistance'] },
      'metabolism': { weight: 0, keywords: ['metabolism', 'slow metabolism', 'thyroid', 'weight gain', 'fatigue', 'energy'] },
      'lupus': { weight: 0, keywords: ['lupus', 'autoimmune', 'butterfly rash', 'flare up', 'chronic pain', 'joint swelling'] },
      'opioid': { weight: 0, keywords: ['opioid', 'recovery', 'sobriety', 'withdrawal', 'addiction', 'detox', 'relapse prevention'] },
      'blood-type-a': { weight: 0, keywords: ['blood type', 'type a', 'sensitive digestion', 'vegetarian diet'] },
      'telomere': { weight: 0, keywords: ['telomere', 'anti-aging', 'longevity', 'cellular health', 'DNA repair'] },
      'unbroken': { weight: 0, keywords: ['unbroken', 'chronic illness', 'fibromyalgia', 'ME/CFS', 'invisible illness', 'chronic fatigue'] },
      'queen': { weight: 0, keywords: ['queen', 'self-love', 'empowerment', 'confidence', 'inner strength', 'healing'] },
      'king': { weight: 0, keywords: ['king', 'leadership', 'strength', 'power', 'focus', 'discipline', 'grounding'] }
    };
    ...
    for (const [condition, data] of Object.entries(conditionMap)) {
      ...
      for (const keyword of data.keywords) {
        if (lowerInput.includes(keyword)) {
          ...
          if (data.keywords.indexOf(keyword) < 5) score += 2;
        }
      }
      if (score > bestScore) {
        bestScore = score;
  ...
  const handleGenerateBlend = async () => {
    ...
    try {
      ...
      if (response.status === 402 || data.preview) {
        ...
        setGenerationError(null);
        return; // Stop here - user must unlock
      }
    ...
    try {
      if (paymentMethod === 'xec') {
        ...
        if (!XUMM_API_KEY || XUMM_API_KEY === 'your-api-key-here' || XUMM_API_KEY.length < 10) {
          throw new Error('Xumm Wallet API key not configured. Check Vercel env vars.');
        }
  ...
  const handleVerifyWallet = async () => {
    ...
    try {
      ...
      if (error.message?.includes('timeout')) {
        ...
        alert('Xumm Wallet API key invalid. Verify NEXT_PUBLIC_XUMM_API_KEY in Vercel.');
      } else {
  ...
  return (
    <div className="min-h-screen bg-black text-white">
      ...
      {/* ✅ Blend Recipe Display with PROPER UNLOCK STATE */}
              ...
              <>
                ...
                {!generatedBlend.preview ? (
                  <>
                    ...
                    >
                      ← Try a predefined blend
                    </button>
                  </>
                ) : (
                  // ✅ Preview Mode: SHOW LOCKED TEASER + UNLOCK BUTTONS
