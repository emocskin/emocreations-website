...
// ✅✅✅ EXTENSIVE CONDITION KEYWORD MAPPING (COMPLETE - SAME AS YOUR WORKING FILE)
...
// ✅✅✅ FIXED: Handle Verify Wallet - Using MODERN PROMISE PATTERN (NOT .on/.off)
...
// ✅ STEP 1: Authorize using PROMISE (Xumm OAuth2 OpenID Connect - Modern Pattern)
let accountAddress;
try {
console.log('🔐 Requesting Xumm authorization...');
// This returns a Promise that resolves with authorization result
const result = await xumm.authorize();
console.log('✅ Authorization result:', result);
// ✅ STEP 2: Extract address from properly structured result
if (!result || !result.account || !result.account.address) {
// Try common variations for different Xumm responses
if (result?.account?.address) {
accountAddress = result.account.address;
} else if (result?.transactionId) {
// Sometimes Xumm only returns transaction ID first
throw new Error('Authorization pending - please approve the request in your Xumm app.');
} else {
throw new Error('Authorization completed but account data not found. Please check popup permissions.');
}
} else {
accountAddress = result.account.address;
}
...
throw new Error('Wallet connection timed out. Please allow popups and try again.');
} else if (authError.message?.includes('popup') || authError.message?.includes('blocked')) {
throw new Error('Please allow popups for emocreations.skin to connect your wallet.');
} else if (authError.message?.includes('invalid') || authError.message?.includes('redirect')) {
throw new Error('❌ Domain Whitelist Issue:\n\nYour domain must be registered in Xumm Developer Console with the correct redirect URI:\n- https://www.emocreations.skin/blend\n- http://localhost:3000/blend\nSee: https://dev.xumm.app/');
}
throw authError;
}
...
} else if (error.message?.includes('Domain Whitelist')) {
alert(error.message);
} else {
